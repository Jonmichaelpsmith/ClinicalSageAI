import { useEffect, useState } from "react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";

export default function CSRAlignmentPanel({ sessionId }) {
  const [alignment, setAlignment] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const loadAlignment = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/sessions/${sessionId}/alignment`);
        
        if (!res.ok) {
          if (res.status === 404) {
            // No alignment report exists yet
            setAlignment(null);
            setError(null);
          } else {
            throw new Error(`Failed to load alignment report: ${res.statusText}`);
          }
        } else {
          const data = await res.json();
          setAlignment(data);
          setError(null);
        }
      } catch (err) {
        console.error("Failed to load alignment report:", err);
        setError(err.message);
      } finally {
        setLoading(false);
      }
    };
    
    if (sessionId) {
      loadAlignment();
    }
  }, [sessionId]);

  if (loading) {
    return (
      <Card className="bg-yellow-50 border border-yellow-200">
        <CardContent className="p-4">
          <p className="text-sm text-yellow-800">Loading alignment data...</p>
        </CardContent>
      </Card>
    );
  }

  if (error) {
    return (
      <Card className="bg-red-50 border border-red-200">
        <CardContent className="p-4">
          <p className="text-sm text-red-800">Error loading alignment data: {error}</p>
        </CardContent>
      </Card>
    );
  }

  if (!alignment) return null;

  return (
    <Card className="bg-yellow-50 border border-yellow-200">
      <CardContent className="p-4 space-y-3">
        <h2 className="text-lg font-semibold text-yellow-900">📊 CSR Semantic Alignment</h2>
        <p className="text-sm text-yellow-800">
          Overall Alignment Score: <strong>{Math.round(alignment.alignment_score * 100)}%</strong>
        </p>

        <table className="text-sm w-full text-left mt-2">
          <thead>
            <tr className="text-xs text-yellow-900 border-b">
              <th className="py-1">Field</th>
              <th>Protocol</th>
              <th>CSR Match</th>
              <th>Score</th>
              <th>Status</th>
            </tr>
          </thead>
          <tbody>
            {alignment.matches.map((item, idx) => (
              <tr key={idx} className="border-b last:border-0">
                <td className="py-1">{item.field}</td>
                <td className="text-yellow-900">{item.protocol_value}</td>
                <td className="text-yellow-900">{item.csr_value}</td>
                <td>{Math.round(item.similarity * 100)}%</td>
                <td>
                  {item.match ? (
                    <span className="text-green-600">✅ Match</span>
                  ) : (
                    <span className="text-red-500">⚠️ Divergent</span>
                  )}
                </td>
              </tr>
            ))}
          </tbody>
        </table>

        <Button
          variant="outline"
          className="mt-4"
          asChild
        >
          <a
            href={`/api/sessions/${sessionId}/alignment`}
            download={`alignment_report_${sessionId}.json`}
            target="_blank"
            rel="noopener noreferrer"
          >
            📥 Download Alignment Report (JSON)
          </a>
        </Button>
      </CardContent>
    </Card>
  );
}