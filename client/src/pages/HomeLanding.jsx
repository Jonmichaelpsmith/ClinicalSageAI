...
// Replaces previous version. Tailwind + i18n + dark‑mode friendly.
...
export default function HomeLanding() {
  ...
  const Button = ({ to, children, variant = "primary" }) => {
    const base =
      "px-6 py-3 rounded-lg inline-flex items-center gap-2 text-lg transition font-medium";
    const variants = {
      primary:
  ...
  return (
    <main className="font-sans text-gray-800 dark:text-gray-100 w-full">
      {/* ================= HERO ================= */}
      <header className="pt-28 pb-24 bg-gradient-to-b from-blue-50 via-white to-white dark:from-slate-900 dark:via-slate-800 dark:to-slate-800 text-center relative overflow-hidden">
        ...
        <p className="relative mt-6 max-w-3xl mx-auto text-xl sm:text-2xl leading-relaxed text-gray-700 dark:text-gray-300">
          {t(
            "The only platform that ingests evidence, auto‑drafts CERs and IND modules, and e‑submits via FDA ESG—cutting CRO billables up to 40%."
          )}
        </p>
      ...
      <section className="bg-blue-600 dark:bg-blue-700 text-white py-14 text-center px-6">
        <h2 className="text-3xl font-bold mb-3">
          {t("Cut external CRO spend • Deliver more studies • Hit filings sooner")}
        </h2>
        <p className="mb-6 opacity-90 max-w-2xl mx-auto">
          {t(
            "Teams running on LumenTrialGuide.AI reduce narrative prep from 6 weeks to 3 days and reinvest those savings into additional indications."
          )}
        </p>
        <Button to="/roi" variant="ghost">
          <BarChart2 size={18} /> {t("Calculate your savings")}
        </Button>
      </section>

      {/* ================= TESTIMONIAL ================= */}
      <section className="py-16 bg-gray-50 dark:bg-slate-900/60 text-center px-6">
        <blockquote className="max-w-3xl mx-auto text-lg italic font-medium">
          {t(
            "\"TrialSage took our IND from patchwork docs to ESG submission in 18 days. Our board approved two extra trials thanks to the savings.\""
          )}
        </blockquote>
        <p className="mt-4 text-sm text-gray-600 dark:text-gray-400">
          — {t("Chief Medical Officer, Precision Oncology Biotech")}
        </p>
      </section>

      {/* ================= FOOTER ================= */}
      <footer className="py-8 bg-white dark:bg-slate-900 text-center text-sm text-gray-500 dark:text-gray-400 border-t border-gray-200 dark:border-gray-800">
        <p>© {year} LumenTrialGuide.AI — {t("All rights reserved")}</p>
      </footer>
    </main>
  );
}