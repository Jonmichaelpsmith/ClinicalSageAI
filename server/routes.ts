  ...
  app.post('/api/planning/init', async (req, res) => {
    try {
      ...
      try {
        ...
        const csrPath = path.join(process.cwd(), 'data/intelligence_db', `${csr_id}.json`);
        ...
        if (fs.existsSync(csrPath)) {
          ...
          // Use docx-templates library (installed with docx) or another approach to generate DOCX
          // As a temporary fallback, we'll create a JSON document instead
          const indData = {
        ...
        console.warn('Error generating fallback IND document:', docError);
